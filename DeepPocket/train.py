'''
DeepPocket CNN Training Script
'''
import sys
import argparse
import os
...
# pylint: disable=E1101,R0914,R0913,R0912,R0915,W0621
...
def parse_args(argv=None):
    ...
    parser.add_argument('-t', '--test_interval', type=int,
                         help="How frequently to test (iterations), default 1000",
                        default=1000)
    ...
    parser.add_argument('--percent_reduced', type=float, default=100,
                        help='''Create a reduced set on the fly based on types file,
                                 using the given percentage:
                                  to use 10 percent pass 10. Range (0,100)''')
    parser.add_argument('--checkpoint', type=str, required=False,
                        help="file to continue training from")
    parser.add_argument('--solver', type=str,
                         help="Solver type. Default is SGD, Nesterov or Adam", default='SGD')
    ...
    parser.add_argument('--step_when', type=int,
                        help="""Perform a dynamic step (reduce base_lr) when training
                             has not improved after this many test iterations, default 15""",
                        default=15)
    ...
    parser.add_argument('--clip_gradients', type=float,
                         default=10.0, help="Clip gradients threshold (default 10)")
...
def initialize_model(model, args):
    ...
    def weights_init(modl):
        ...
        if isinstance(modl, nn.Conv3d):
            torch.nn.init.xavier_uniform_(modl.weight)
...
def get_model_gmaker_eproviders(args):
    '''
    Prepare model, gridmaker, and eproviders, train and test sets
    '''
    # train example provider
    eptrain_l = molgrid.ExampleProvider(shuffle=True, stratify_receptor=True,
                                     labelpos=0,balanced=True,
                                      data_root=args.data_dir,recmolcache=args.train_recmolcache)
    eptrain_l.populate(args.train_types)
    # test example provider
    eptest_large_l = molgrid.ExampleProvider(shuffle=False,
                                    stratify_receptor=False, labelpos=0,balanced=False,
                                     data_root=args.data_dir,
                                     iteration_scheme=molgrid.IterationScheme.LargeEpoch,
                                     default_batch_size=args.batch_size,
                                     recmolcache=args.test_recmolcache)
    eptest_large_l.populate(args.test_types)
    eptest_small_l = molgrid.ExampleProvider(shuffle=True, stratify_receptor=True,
                                        labelpos=0, balanced=True,
                                        data_root=args.data_dir,
                                        iteration_scheme=molgrid.IterationScheme.SmallEpoch,
                                        default_batch_size=args.batch_size,
                                        recmolcache=args.test_recmolcache)
    eptest_small_l.populate(args.test_types)
    # gridmaker with defaults
    gmaker_l = molgrid.GridMaker()
    # dims = gmaker_l.grid_dimensions(eptrain_l.num_types())
    model_file = imp.load_source("model", args.model)
    # load model with seed
    torch.manual_seed(args.seed)
    model_m = model_file.Model()

    return model_m, gmaker_l, eptrain_l, eptest_large_l,eptest_small_l

def train_and_test(args, model, eptrain, eptest_large,eptest_small, gmaker):
    '''
    Training and testing of model with the given training and test sets
    '''
    def test_model(model, epo, gmaker, _, batch_size):
        '''
        ...
        for batch in epo:
            ...
            #  other 14 are for ligand and will remain 0.
    ...
    for i in range(initial, iterations):
        ...
        if i % test_interval == 0 and i!=0:
            test_loss, test_accuracy,_,_,_ = test_model(model,
                                     eptest_small, gmaker,percent_reduced, batch_size)
            _, _, test_auc, _, _ = test_model(model, eptest_large, gmaker,
            ...
            if test_loss < bests['test_loss']:
                ...
                torch.save({'model_state_dict': model.state_dict(),
                            ...
                            'Bests': bests,
                            'Iteration': i + 1}, outprefix +
                            '_best_test_loss_' + str(i + 1) + '.pth.tar')
            ...
            if test_accuracy > bests['test_accuracy']:
                ...
                torch.save({'model_state_dict': model.state_dict(),
                                ...
                                'Bests': bests,
                                'Iteration': i + 1}, outprefix +
                                 '_best_test_accuracy_' + str(i + 1) + '.pth.tar')
            ...
            if test_auc > bests['test_auc']:
                ...
                torch.save({'model_state_dict': model.state_dict(),
                            ...
                            'Bests': bests,
                            'Iteration': i + 1}, outprefix +
                             '_best_test_auc_' + str(i + 1) + '.pth.tar')
            ...
            if (i -bests['train_iteration'] >= args.step_when and optimizer.param_groups[0]['lr'] <=
            ...
            print("Iteration {}, total_test_loss: {:.3f},total_test_accuracy: {:.3f},total_test_auc: {:.3f}, Best_test_loss: {:.3f},Best_test_accuracy: {:.3f},Best_test_auc: {:.3f},learning_Rate: {:.7f}".format(
                    i + 1, test_loss, test_accuracy,test_auc,bests['test_loss'],
                    bests['test_accuracy'],bests['test_auc'], optimizer.param_groups[0]['lr']))
